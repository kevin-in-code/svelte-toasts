...
<ul>
  ...
  <li>
    <Toast
      ...
      on:previous={() => { (index - 1 >= 0) && items[index - 1]?.ref?.focus?.(); } }
      on:next={() => { (index + 1 < items.length) && items[index + 1]?.ref?.focus?.(); } }
      on:move={() => { (index + 1 < items.length) ? items[index + 1]?.ref?.focus?.() : items[index - 1]?.ref?.focus?.(); } }
    />
  </li>
  {/each}
</ul>
