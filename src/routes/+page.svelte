    ...
    if (context) {
      ...
      context.raise({
        ...
        body: [
          ...
          "It has some nice features. For example, it's themes are nice, and they work well on both light and dark pages.",
        ],
        duration: 5000,
      });
      context.raise({
...
<ToastContextProvider bind:context defaultPosition="center" {theme}>
  ...
  <fieldset>
    ...
    <legend>Category</legend> -->
      <div role="group" class="row">
        <button
          class:outline={categoryOption !== 'Success'}
          on:click={() => (categoryOption = 'Success')}>Success</button
        >
        <button
          class:outline={categoryOption !== 'Information'}
          on:click={() => (categoryOption = 'Information')}>Information</button
        >
        <button
          class:outline={categoryOption !== 'Warning'}
          on:click={() => (categoryOption = 'Warning')}>Warning</button
        >
        <button
          class:outline={categoryOption !== 'Error'}
          on:click={() => (categoryOption = 'Error')}>Error</button
        >
      ...
      <fieldset>
        ...
        <div class="row">
          ...
          <input
            type="text"
            id="title-custom-text"
            placeholder="Custom title text..."
            disabled={customTitleOption !== 'Custom'}
          />
        </div>
      </fieldset>
    </div>
